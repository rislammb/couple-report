...
const UnionTotalRange = ({
  ...
  const getCellValue = (rowName, name) => {
    const rowCellData = [
      ...zeroRange.map((row) => row[rowName][name]),
      ...twentyRange.map((row) => row[rowName][name]),
      ...thirtyRange.map((row) => row[rowName][name]),
      ...fortyRange.map((row) => row[rowName][name]),
    ];

    return getTotalFromArray(rowCellData);
  };
  useEffect(() => {
    if (zeroRange && twentyRange && thirtyRange && fortyRange) {
      setRowOne((prev) => {
        return {
          ...prev,
